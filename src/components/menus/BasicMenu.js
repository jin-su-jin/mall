...
const BasicMenu = () => {
    ...
    return ( 
        <nav id='navbar' className=" flex bg-blue-300">
            ...
            {loginState.email ? 
                <div className="text-white text-sm m-1 rounded" ><Link to={'/member/logout'}>Logut</Link></div>
                :
                <div className="text-white text-sm m-1 rounded" ><Link to={'/member/login'}>Login</Link></div>
            }
            </div>
        </nav>
    );
